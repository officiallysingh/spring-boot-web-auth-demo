...
public class ActuatorEndpointProperties {
    ...
    static final List<String> ALL_ENDPOINTS = Arrays.asList("auditevents", "beans", "caches", "conditions", "configprops", "env", "flyway", "health", "heapdump", "httptrace", "info", "integrationgraph", "liquibase", "logfile", "loggers", "mappings", "metrics", "prometheus", "scheduledtasks", "sessions", "shutdown", "threaddump");
    ...
    public String[] getPaths() {
        ...
        if (SLASH.equals(this.basePath)) {
            return exposedEndpoints.stream().map(path -> SLASH + this.pathMapping.getOrDefault(path, path) + "/**").toArray(String[]::new);
        } else {
